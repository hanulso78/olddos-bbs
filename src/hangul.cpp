...
****************************************************************************
  Example     :
	...
	check_hangul_code(t, strlen(t), &ks_count, &kssm_count, &eng_count);
...
//---------------------------------------------------------------------------
static int
check_hangul_range( int hancode, int HexStart, int HexEnd, int max )
{
	int i;
	for( i = 0; i < max; i++ )
	{
...
static void *
my_bsearch(const void *p, const void *q, int count, int size,
...
static int code_compare(const void *p,const void *q)
{
    return ((int*)q)[0]-((int*)p)[0];
}

#define MAX_CODE  48
int is_not_ks_code( int hancode )
{
	...
	if( (int *)my_bsearch(&hancode, kssmcode, MAX_CODE, sizeof(int), code_compare) != NULL ||
...
int check_hangul_code( const char *buf, int len, int *ks_buf, int *ksm_buf, int *eng_buf )
{
	...
	for( i = 0; i < max; i++ )
	{
		...
		if( ( (hancode > 0xa1a2 && hancode < 0xa1ff) ||
					...
					// KS5061의 한글코드인가?
					check_hangul_range( hancode, 0xb0a0, 0xb0ff, 16 ) ||
					check_hangul_range( hancode, 0xc0a0, 0xc0ff, 9  ) ||

					( hancode == 0xb8ae ) ||
					( hancode == 0xb7e7 ) || // 완성형 '루'

					// KS5061의 한자코드인가?
					check_hangul_range( hancode, 0xcaa0, 0xcaff, 6  ) ||
					check_hangul_range( hancode, 0xd0a0, 0xd0ff, 16 ) ||
					check_hangul_range( hancode, 0xe0a0, 0xe0ff, 16 ) ||
					check_hangul_range( hancode, 0xf0a0, 0xf0ff, 14 ) ) &&

					//  1998.08.22 수정(Cherie)
					//  조합형과 완성형 둘다 사용되는 코드중, 조합형에서 자주
					//  사용되는 코드는 강제로 조합형 코드로 간주한다.
					!is_not_ks_code( hancode ) )
					{
						if(++ks_count >= (len/3) )
							break;
					}
